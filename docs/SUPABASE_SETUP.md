...
4. **`003_assessment_plan_formal_pdf.sql`（個別支援計画書・アセスメント用）**：アセスメント項目（生年月日・生活意向など）および `saved_programs.mapped_plan`（様式自動マッピングのスナップショット）を追加します。**PDF様式機能を使う場合は必ず適用してください。**

5. エラーが出た場合は、メッセージ全文を確認してください（既にテーブルがある場合は `DROP TABLE` が必要になることがあります。本番ではマイグレーション管理を推奨します）。
