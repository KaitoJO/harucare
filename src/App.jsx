...
# 障害種別別の支援原理リファレンス（必要に応じて根拠に活用）
- 自閉スペクトラム症：構造化・視覚優位性の活用・予測可能性・興味の活用・感覚特性への配慮・予告徹底
- ADHD：実行機能の外部化（ツール・チェックリスト）・短時間集中・自己選択の尊重・自己肯定感保護・即時フィードバック
- 発達遅滞／知的障害：発達段階優先・AAC（補助代替コミュニケーション）・反復学習・成功体験の蓄積・身辺自立優先
- ダウン症：強み（社会性・模倣力）の活用・健康面（心疾患・呼吸器）配慮・段階的指導・実物提示
- 脳性麻痺：姿勢保持の安定→粗大運動→微細運動の順・装具/補助具の活用・疲労に配慮した活動量設計
- 重症心身障害：感覚刺激の質と量・生活リズム・医療的ケアとの整合・呼吸/嚥下安全・コミュニケーションサインの読み取り
- 言語障害：受容言語と表出言語の評価分離・代替手段（絵カード・身振り）・本人ペースの待ち時間確保
- 学習障害（LD）：得意な認知ルートの特定・苦手領域の代替手段・自己理解の促進・二次障害予防
- 医療的ケア児：医療連携の優先・体調による活動調整・家族の負担軽減・きょうだい児への配慮

# 出力スキーマ（厳守・全フィールド必須）

各活動・各目標・各支援内容について、必ず**「理由：」または「（理由：〜）」**という形式で根拠を併記すること。
根拠が無い項目は出力しないこと。「Aさんに適切なため」のような抽象表現は禁止。

# 根拠を書くときの必須要素
1. その障害種別の発達原理・支援理論（上記リファレンス参照）
2. 本児の固有の強み・課題・備考に紐づける
...
- 「専門家の判断により」などの権威依存表現
- 「理由」「根拠」フィールドの省略・空欄
- 障害種別を問わず汎用的な記述（必ずその障害特有の根拠で書くこと）`;
...
- バランス遊び・トンネルくぐり・平均台（理由：発達遅滞児に多い体幹の弱さに対し、楽しく姿勢保持筋を鍛えるため。座学より動きのある課題で注意も保ちやすい）
- シール貼り・型はめ・パズル（理由：手指協調を伸ばす段階的課題。シールは把持→剥がす→貼るの一連動作で巧緻性を、型はめは形の弁別を、パズルは全体把握を育てる）

支援のポイントと理由：
- 動作を短く区切る（理由：発達遅滞児の注意持続時間に合わせ、成功体験を切れ目なく積めるようにするため）
- 視覚提示は常に同じ位置・形式（理由：認知負荷を下げ、見通しを持たせることで活動への安心感を作るため）
- 短時間で完了できる課題から始める（理由：発達の最近接領域に合わせ、達成感を起点に次の挑戦動機を引き出すため）
- 成功したらすぐほめる（理由：強化原理。本児は言語理解が限定的なため、即時の表情・身振りでの肯定が最も伝わる）

このように、すべての活動・支援に「理由：〜」を併記し、なぜその選択をしたのかが行政の実地指導で即答できる粒度で書かれていることが、HaruCareの計画書の必須要件です。`;
...
本児の障害特性と発達段階に紐づけて3〜5行で記述すること。

## 2. 支援方針（なぜこの方針か）
方針を1〜2行で示した上で、「（理由：〜）」で根拠を3〜5行記述すること。
...
理由が書かれていない活動は出力しないこと。例：
- バランス遊び・平均台（理由：${child.disability}の特性として体幹保持が弱く、楽しい動作で姿勢筋を鍛えるため）

## 5. 支援のポイント（5項目以上・必ず各項目に「理由：」併記）
- ポイント：（具体的に）
- 理由：（${child.disability}の支援原理＋本児固有情報に紐づけて）

## 6. 家庭との連携
連携内容と「なぜその連携が必要か」の理由を併記。
...
- 見出しは ## または ■ を使い、Markdown記法で読みやすく整形すること${extra}`;
}
